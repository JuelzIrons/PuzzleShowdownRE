namespace Unity.Networking.Transport
{
	internal static class HMACSHA256
	{
		public unsafe static void ComputeHash(byte* keyValue, int keyArrayLength, byte* messageBytes, int messageLength, byte* result)
		{
			...
			global::Unity.Networking.Transport.SHA256.SHA256State sHA256State = global::Unity.Networking.Transport.SHA256.SHA256State.Create();
			...
			{
				ptr2[i] = (byte)(0x36 ^ keyValue[i]);
			}
			for (int j = keyArrayLength; j < 64; j++)
			{
				ptr2[j] = 54;
			}
			sHA256State = global::Unity.Networking.Transport.SHA256.SHA256State.Create();
