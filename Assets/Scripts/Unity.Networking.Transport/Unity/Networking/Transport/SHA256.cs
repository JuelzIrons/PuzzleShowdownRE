namespace Unity.Networking.Transport
{
	internal static class SHA256
	{
		internal struct SHA256State
		{
			...
			public unsafe static global::Unity.Networking.Transport.SHA256.SHA256State Create()
			{
				global::Unity.Networking.Transport.SHA256.SHA256State result = new global::Unity.Networking.Transport.SHA256.SHA256State
				{
					state = 
					{
						FixedElementField = 1779033703u
					}
				};
			...
			private unsafe void Transform(uint* data)
			{
				...
				for (int j = 0; j < 64; j += 16)
				{
					for (int k = 0; k < 16; k++)
					{
						ptr2[(7 - k) & 7] += S(ptr2[(4 - k) & 7]) + Ch(ptr2[(4 - k) & 7], ptr2[(5 - k) & 7], ptr2[(6 - k) & 7]) + K[k + j] + ((j != 0) ? (ptr[k & 0xF] += s(ptr[(k - 2) & 0xF]) + ptr[(k - 7) & 0xF] + s2(ptr[(k - 15) & 0xF])) : (ptr[k] = data[k]));
						ptr2[(3 - k) & 7] += ptr2[(7 - k) & 7];
						ptr2[(7 - k) & 7] += S2(ptr2[-k & 7]) + Maj(ptr2[-k & 7], ptr2[(1 - k) & 7], ptr2[(2 - k) & 7]);
					}
				}
				for (int l = 0; l < 8; l++)
				{
				...
				static uint ROTR(uint x, byte n)
				{
					return (x << 32 - n) | (x >> (int)n);
				}
				static uint S(uint x)
				{
					return ROTR(x, 6) ^ ROTR(x, 11) ^ ROTR(x, 25);
				}
				static uint S2(uint x)
				{
