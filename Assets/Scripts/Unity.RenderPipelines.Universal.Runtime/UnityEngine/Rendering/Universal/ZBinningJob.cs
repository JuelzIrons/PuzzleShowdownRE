namespace UnityEngine.Rendering.Universal
{
	...
	internal struct ZBinningJob : global::Unity.Jobs.IJobFor
	{
		...
		public global::Unity.Collections.NativeArray<uint> bins;
		...
		public void Execute(int jobIndex)
		{
			...
			int num3 = 128 * num;
			int num4 = global::Unity.Mathematics.math.min(num3 + 128, binCount) - 1;
		...
		private void FillZBins(int binStart, int binEnd, int itemStart, int itemEnd, int headerIndex, int itemOffset, int binOffset)
		{
			...
			{
				global::Unity.Mathematics.float2 float5 = minMaxZs[itemOffset + i];
				int num = global::Unity.Mathematics.math.max((int)((isOrthographic ? float5.x : global::Unity.Mathematics.math.log2(float5.x)) * zBinScale + zBinOffset), binStart);
				int num2 = global::Unity.Mathematics.math.min((int)((isOrthographic ? float5.y : global::Unity.Mathematics.math.log2(float5.y)) * zBinScale + zBinOffset), binEnd);
				int num3 = i / 32;
				...
				for (int j = num; j <= num2; j++)
				{
					...
					item = global::Unity.Mathematics.math.min(item, (uint)i);
					item2 = global::Unity.Mathematics.math.max(item2, (uint)i);
