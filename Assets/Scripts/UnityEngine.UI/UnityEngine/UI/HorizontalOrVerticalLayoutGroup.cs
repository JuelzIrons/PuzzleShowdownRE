	...
	public abstract class HorizontalOrVerticalLayoutGroup : global::UnityEngine.UI.LayoutGroup
	{
		...
		protected void CalcAlongAxis(int axis, bool isVertical)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (flag2)
				{
					num2 = global::UnityEngine.Mathf.Max(min + num, num2);
					num3 = global::UnityEngine.Mathf.Max(preferred + num, num3);
					num4 = global::UnityEngine.Mathf.Max(flexible, num4);
				}
				else
				{
					num2 += min + spacing;
			...
			num3 = global::UnityEngine.Mathf.Max(num2, num3);
		...
		protected void SetChildrenAlongAxis(int axis, bool isVertical)
		{
			...
			if (num2)
			{
				...
				for (int i = num3; m_ReverseArrangement ? (i >= num4) : (i < num4); i += num5)
				{
					...
					float num7 = global::UnityEngine.Mathf.Clamp(value, min, (flexible > 0f) ? num : preferred);
			...
			{
				t = global::UnityEngine.Mathf.Clamp01((num - GetTotalMinSize(axis)) / (GetTotalPreferredSize(axis) - GetTotalMinSize(axis)));
			}
			for (int j = num3; m_ReverseArrangement ? (j >= num4) : (j < num4); j += num5)
			{
				global::UnityEngine.RectTransform rectTransform2 = base.rectChildren[j];
				...
				float num13 = global::UnityEngine.Mathf.Lerp(min2, preferred2, t);
		...
		private void GetChildSizes(global::UnityEngine.RectTransform child, int axis, bool controlSize, bool childForceExpand, out float min, out float preferred, out float flexible)
		{
			...
			{
				min = global::UnityEngine.UI.LayoutUtility.GetMinSize(child, axis);
				preferred = global::UnityEngine.UI.LayoutUtility.GetPreferredSize(child, axis);
				flexible = global::UnityEngine.UI.LayoutUtility.GetFlexibleSize(child, axis);
			}
			if (childForceExpand)
			{
				flexible = global::UnityEngine.Mathf.Max(flexible, 1f);
			}
		}
	}
}
