namespace UnityEngine.InputSystem.LowLevel
{
	internal struct InputEventStream
	{
		...
		public unsafe global::UnityEngine.InputSystem.LowLevel.InputEvent* currentEventPtr
		{
			get
			{
		...
		public unsafe uint numBytesRetainedInBuffer => (uint)((byte*)m_CurrentNativeEventWritePtr - (byte*)global::Unity.Collections.LowLevel.Unsafe.NativeArrayUnsafeUtility.GetUnsafeBufferPointerWithoutChecks(m_NativeBuffer.data));
		...
		{
			m_CurrentNativeEventWritePtr = (m_CurrentNativeEventReadPtr = (global::UnityEngine.InputSystem.LowLevel.InputEvent*)global::Unity.Collections.LowLevel.Unsafe.NativeArrayUnsafeUtility.GetUnsafeBufferPointerWithoutChecks(eventBuffer.data));
			...
			m_AppendBuffer = default(global::UnityEngine.InputSystem.LowLevel.InputEventBuffer);
			m_RemainingAppendEventCount = 0;
			m_MaxAppendedEvents = maxAppendedEvents;
			m_IsOpen = true;
		}

		public unsafe void Close(ref global::UnityEngine.InputSystem.LowLevel.InputEventBuffer eventBuffer)
		{
			if (m_NumEventsRetainedInBuffer > 0)
			{
				void* unsafeBufferPointerWithoutChecks = global::Unity.Collections.LowLevel.Unsafe.NativeArrayUnsafeUtility.GetUnsafeBufferPointerWithoutChecks(m_NativeBuffer.data);
				long num = (byte*)m_CurrentNativeEventWritePtr - (byte*)unsafeBufferPointerWithoutChecks;
				m_NativeBuffer = new global::UnityEngine.InputSystem.LowLevel.InputEventBuffer((global::UnityEngine.InputSystem.LowLevel.InputEvent*)unsafeBufferPointerWithoutChecks, m_NumEventsRetainedInBuffer, (int)num, (int)m_NativeBuffer.capacityInBytes);
			}
			else
			{
		...
		public unsafe void Write(global::UnityEngine.InputSystem.LowLevel.InputEvent* eventPtr)
		{
			...
			{
				global::UnityEngine.Debug.LogError("Maximum number of queued events exceeded. Set the 'maxQueuedEventsPerUpdate' setting to a higher value if you need to queue more events than this. " + $"Current limit is '{m_MaxAppendedEvents}'.");
				return;
			}
			bool isCreated = m_AppendBuffer.data.IsCreated;
			byte* data = (byte*)m_AppendBuffer.bufferPtr.data;
			m_AppendBuffer.AppendEvent(eventPtr, 2048, global::Unity.Collections.Allocator.Temp);
			if (!isCreated)
			{
				m_CurrentAppendEventWritePtr = (m_CurrentAppendEventReadPtr = (global::UnityEngine.InputSystem.LowLevel.InputEvent*)global::Unity.Collections.LowLevel.Unsafe.NativeArrayUnsafeUtility.GetUnsafeBufferPointerWithoutChecks(m_AppendBuffer.data));
			}
			else
			{
				...
				if (data != data2)
				{
					...
					m_CurrentAppendEventWritePtr = (global::UnityEngine.InputSystem.LowLevel.InputEvent*)(data2 + num);
					m_CurrentAppendEventReadPtr = (global::UnityEngine.InputSystem.LowLevel.InputEvent*)(data2 + num2);
				}
			}
		...
		public unsafe global::UnityEngine.InputSystem.LowLevel.InputEvent* Peek()
		{
			...
			{
				return global::UnityEngine.InputSystem.LowLevel.InputEvent.GetNextInMemory(m_CurrentNativeEventReadPtr);
			}
			if (m_RemainingNativeEventCount == 1)
			{
				if (m_RemainingAppendEventCount <= 0)
			...
			{
				return global::UnityEngine.InputSystem.LowLevel.InputEvent.GetNextInMemory(m_CurrentAppendEventReadPtr);
			}
			return null;
		}
	}
