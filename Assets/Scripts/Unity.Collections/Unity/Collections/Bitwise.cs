namespace Unity.Collections
{
	...
	internal struct Bitwise
	{
		...
		{
			return global::Unity.Mathematics.math.lzcnt((uint)value) - 24;
		}

		internal static int tzcnt(byte value)
		{
			return global::Unity.Mathematics.math.min(8, global::Unity.Mathematics.math.tzcnt((uint)value));
		}

		internal static int lzcnt(ushort value)
		{
			return global::Unity.Mathematics.math.lzcnt((uint)value) - 16;
		}

		internal static int tzcnt(ushort value)
		{
			return global::Unity.Mathematics.math.min(16, global::Unity.Mathematics.math.tzcnt((uint)value));
		}
		...
		private unsafe static int FindUlong(ulong* ptr, int beginBit, int endBit, int numBits)
		{
			...
			for (int num2 = AlignUp(endBit, num) / num; i < num2; i++)
			{
				...
				int num3 = i * num;
				int num4 = global::Unity.Mathematics.math.min(num3 + num, endBit) - num3;
				if (num3 != beginBit)
				{
					ulong x = ptr[num3 / num - 1];
					int num5 = global::Unity.Mathematics.math.max(num3 - global::Unity.Mathematics.math.lzcnt(x), beginBit);
					num4 += num3 - num5;
					num3 = num5;
				}
				for (i++; i < num2; i++)
				{
					...
					ulong num6 = ptr[i];
					int num7 = i * num;
					num4 += global::Unity.Mathematics.math.min(num7 + global::Unity.Mathematics.math.tzcnt(num6), endBit) - num7;
					if (num6 != 0L)
					{
						break;
					}
		...
		private unsafe static int FindUshort(ulong* ptr, int beginBit, int endBit, int numBits)
		{
			...
			for (int num2 = AlignUp(endBit, num) / num; i < num2; i++)
			{
				...
				if (num3 != beginBit)
				{
					...
					int num5 = global::Unity.Mathematics.math.max(num3 - lzcnt(value), beginBit);
					num4 += num3 - num5;
					num3 = num5;
				}
				for (i++; i < num2; i++)
				{
					...
					int num7 = i * num;
					num4 += global::Unity.Mathematics.math.min(num7 + tzcnt(num6), endBit) - num7;
					if (num6 != 0)
					{
						break;
					}
		...
		private unsafe static int FindByte(ulong* ptr, int beginBit, int endBit, int numBits)
		{
			...
			for (int num2 = AlignUp(endBit, num) / num; i < num2; i++)
			{
				...
				for (i++; i < num2; i++)
				{
					...
					byte b = ((byte*)ptr)[i];
					int num6 = i * num;
					num4 += global::Unity.Mathematics.math.min(num6 + tzcnt(b), endBit) - num6;
					if (b != 0)
					{
						break;
					}
		...
		private unsafe static int FindUpto14bits(ulong* ptr, int beginBit, int endBit, int numBits)
		{
			...
			for (int i = num2; i < num3; i++)
			{
				...
				{
					int num4 = i * 8;
					int num5 = global::Unity.Mathematics.math.min(num4 + tzcnt(b3), endBit) - num4;
					if (num + num5 >= numBits)
					{
						return num4 - num;
					}
					num = lzcnt(b3);
					int num6 = num4 + 8;
					int num7 = global::Unity.Mathematics.math.max(num6 - num, beginBit);
					num = global::Unity.Mathematics.math.min(num6, endBit) - num7;
					if (num >= numBits)
					{
						return num7;
					}
		...
		internal unsafe static int CountBits(ulong* ptr, int length, int pos, int numBits = 1)
		{
			...
			{
				ulong num8 = num6 & num7;
				return global::Unity.Mathematics.math.countbits(ptr[num2] & num8);
			}
			int num9 = global::Unity.Mathematics.math.countbits(ptr[num2] & num6);
			for (int i = num2 + 1; i < num4; i++)
			{
				num9 += global::Unity.Mathematics.math.countbits(ptr[i]);
			}
			return num9 + global::Unity.Mathematics.math.countbits(ptr[num4] & num7);
		}
